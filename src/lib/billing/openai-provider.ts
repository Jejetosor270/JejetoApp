...
const instructions = `Extract facts from a client quote/devis or client invoice for an internal procurement ERP. Return only the strict schema. Never guess. Mark every observation EXTRACTED, MISSING, or AMBIGUOUS. AMBIGUOUS needs a short diagnostic and normally a null value. Money is a non-negative decimal string without separators, rates are fractions (0.20 means 20%), and dates are YYYY-MM-DD. Extract HT, every VAT line, total VAT, and TTC independently. Extract freightCoverageHt as the explicit HT portion already included in totalHt for freight, transport, shipping or delivery charges. Do not add it to totalHt again. When absent or unclear mark MISSING or AMBIGUOUS; never infer it from a markup or allocate it to an Order. Do not average multiple VAT rates. Preserve payment wording and propose installments only where supported by the document. Do not decide legal VAT treatment, FX rates, authoritative Client, Project, Order allocation, or invoice matching.`;

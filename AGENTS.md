...
- Project financial performance compares full-Project planning targets with actual-to-date active Client Invoice HT and authoritative economic costs, including Project-level freight expenses once. Client Budget, Order planned sell, billing allocations, and cash timing are not actual revenue.
- Project VAT position is active Client Invoice output VAT minus deductible Order and Project-freight input VAT in Project reporting currency. Partial recoverability uses the stored recoverable rate; non-deductible VAT remains economic cost, and missing required FX makes the VAT position incomplete.
